...
public class SavingAc implements ActionListener, KeyListener
{
	Connection con;
...
public SavingAc()
{
f = new JFrame ("Saving Account Form");
frame = new JFrame();

p1 = new JPanel();
lb_heading = new JLabel("New Saving Account Form");
...
lb_docs = new JLabel("Document Enclosed (Choose only one):");
...
{
JOptionPane.showMessageDialog(f, "Please Enter only integer values", "Error", JOptionPane.ERROR_MESSAGE);
...
{
	String account=null, date, name, f_name, add, dob, contact, nom_name, nom_add, nom_contact, open_bal, doc=null;
	String Ldeposit="0";
	String Lwithdraw="0";
	String Tdeposit="0";
	String Twithdraw="0";
	String clos_bal="1000";
	...
	if(ae.getSource()==btn_submit)
	{
		...
		{       /*
			...
			rs = stmt.executeQuery("Select Top 1 Account_No from saving order by Account_No desc");
			while (rs.next())
			{
			...
			con.close();
				...
				else
				{
					Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
					con = DriverManager.getConnection("jdbc:odbc:bank");
					ps = con.prepareStatement("Insert into saving values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
					...
					con = DriverManager.getConnection("jdbc:odbc:bank");
					stmt = con.createStatement();
					ResultSet rs1 = stmt.executeQuery("Select Top 1 * from saving order by Account_No desc");
...
save.launchFrame();
tf_date.setText(save.now("dd.MM.yyyy"));
