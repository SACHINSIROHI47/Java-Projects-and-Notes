...
import java.sql.*;
public class ProductInfo extends JApplet implements ActionListener
{
...
con = DriverManager.getConnection("jdbc:odbc:MyDataSource","sa","rgpg007");
...
/*Remove all the elements from the idCombo JcomboBox object*/
 ...
 if(ae.getActionCommand() == "Insert")
 {
  //Insert a row in the product table
	...
	{
		JOptionPane.showMessageDialog(null,"Enter Product Rate");
	...
	else if(unit_of_msr.length()==0)
	{
		JOptionPane.showMessageDialog(null,"Enter Product Unit of Measurement");
		unit_of_msrField.requestFocus();
	}
	else
	{
  try
...
quantityField.setText(Integer.toString(rs.getInt(3)));    unit_of_msrField.setText(rs.getString(4));
    	con.close();
   	}
  	catch(Exception e)
  	{
